...
export const MembersStep = ({
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="relative">
            <SearchBar onSearch={onSearch} onChange={onSearch} placeholder="Search users..." />
            {searchResults.length > 0 && (
                <UserSearchDropdown results={searchResults} onSelect={onSelect} onClose={() => onSearch("")} isLoading={isLoading} />
            )}
        </div>
        ...
        <div className="bg-background-muted/30 rounded-xl p-4 border border-border min-h-[300px] flex flex-col">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar">
                {addedUsers.length === 0 ? (
                    <p className="text-sm text-muted-foreground italic text-center mt-10">No users added yet</p>
                ) : (
                    addedUsers.map((user: any) => (
                        <UserShortRow key={user.userId} user={user} action={
                            <Button onClick={() => onRemove(user.userId)} className="text-danger p-1">
