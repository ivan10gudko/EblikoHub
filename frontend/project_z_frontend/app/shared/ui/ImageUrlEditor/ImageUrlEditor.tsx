...
export const ImageUrlEditor = ({
  ...
  const containerClasses = variant === 'portrait' 
    ? "h-[280px] w-[190px]" 
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full">
      <div className="relative group">
        <div className={`${containerClasses} bg-card/50 backdrop-blur-sm rounded-2xl border-2 border-dashed border-border flex items-center justify-center overflow-hidden shadow-xl transition-all duration-300 group-hover:border-primary/50 group-hover:shadow-primary/10`}>
          <img
            src={imageError || !imageUrl ? defaultImage : imageUrl}
            className={`h-full w-full object-cover transition-transform duration-500 group-hover:scale-105 ${!imageUrl || imageError ? "opacity-20 grayscale" : ""
              }`}
            alt="Title Preview"
      ...
      <div className="w-full space-y-3">
        ...
        <div className="relative group/input">
          <Input
            ...
            className={`h-12 w-full pl-4 pr-12 border-2 rounded-xl font-medium bg-background/50 transition-all shadow-inner ${imageError && tempUrl
