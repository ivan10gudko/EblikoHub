...
export const PinnedWatchlistRowReadOnly = ({ title }: PinnedWatchlistRowReadOnlyProps) => {
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 bg-primary/5 p-3 rounded-xl border-2 border-primary/40 shadow-md transition-all duration-300 w-full relative ${themeClasses}`}
    >
      ...
      <div className="flex items-center flex-1 gap-3 min-w-0">
        {/* Кнопка Unpin гостю непотрібна, тому тут одразу йде обкладинка */}
      ...
      <div className="flex items-center justify-between sm:justify-end gap-3 sm:w-auto mt-2 sm:mt-0">
        <div className="flex-shrink-0 px-3 py-1.5 bg-transparent text-primary text-[10px] font-black uppercase tracking-wider rounded-lg border border-primary/30">
        ...
        {/* Меню дій доступне (кнопка видалення всередині сховається автоматично завдяки isOwn={false}) */}
