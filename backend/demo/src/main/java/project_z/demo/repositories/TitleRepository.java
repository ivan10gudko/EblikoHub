...
public interface TitleRepository extends JpaRepository<TitleEntity, Long>,
    ...
    @Query(value = """
                ...
                WHERE LOWER(r.name) = LOWER(:category)
                  AND (r.value > :currentRating OR (r.value = :currentRating AND t.title_id > :currentId))
                  AND t.title_id != :currentId
                ORDER BY r.value ASC, t.title_id ASC
                LIMIT 2
            """, nativeQuery = true)
    List<Object[]> findCloserTitlesUp(@Param("currentId") Long currentId,
            @Param("category") String category,
            @Param("currentRating") Float currentRating);
    ...
    @Query(value = """
                ...
                LIMIT 2
            """, nativeQuery = true)
    List<Object[]> findCloserTitlesDown(@Param("currentId") Long currentId,
            @Param("category") String category,
            @Param("currentRating") Float currentRating);
}
