...
public class UserFavoriteTitleServiceImpl implements UserFavoriteTitleService {
    ...
    public UserProfileDto addTitleToFavorite(UUID userId, Long titleId, Integer position) {
        ...
        TitleEntity title = titleRepository.findById(titleId).orElseThrow(
        ...
        if (!alreadyExists) {
            if (position == null || position < 1 || position > appConfig.getMaxFavoriteTitles()) {
                throw new IllegalArgumentException(
                        "Invalid position: must be between 1 and " + appConfig.getMaxFavoriteTitles());
            }
            ...
            if (currentFavoritesCount >= appConfig.getMaxFavoriteTitles()) {
                throw new UserFavoriteTitlesLimitReachedException(
                        "You have reached the maximum limit of favorite titles ("
        ...
        UserProfileDto res = userService.getUserProfile(userId, null); // null because of there is no need of friendship status
